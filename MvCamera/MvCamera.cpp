...
void Mv_Camera::Init_Camera()
{
    ...
    int nums = 2;            // 存储相机数量
    if (CameraEnumerateDevice(&infos, &nums) != CAMERA_STATUS_SUCCESS)
    {
        Init_State = false;
        return;
    }
    // 枚举所有的相机设备并将其信息写入infos，相机数量写入nums
    Camera_Name = std::string(infos.acFriendlyName); // 录入相机名称
    if (CameraInit(&infos, -1, -1, &handle) != CAMERA_STATUS_SUCCESS)
    {
        Init_State = false;
        return;
    } // 初始化相机，此时会分配相机的句柄
...
bool Mv_Camera::Open_Camera()
{
    auto status = CameraPlay(handle); // 开启相机接受图像
